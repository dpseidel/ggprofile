...
pngs <- list.files("results/", pattern = "png", full.names = T)
png_timing <- purrr::map_df(pngs, read_csv) %>%
  mutate(device = rep(c("cairo", "CairoPNG", "quartz", "Xlib", "win"), each = 101))
```
...
png_timing %>%
  ...
  facet_grid(. ~ parse_factor(device, levels = c("cairo", "quartz", "Xlib", "CairoPNG", "win"))) +
